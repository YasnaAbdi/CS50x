...
WHERE atm_location = 'Leggett Street' AND a.year = 2023 AND a.month = 7 AND a.day = 28 AND a.transaction_type = 'withdraw';
--phone call investigation--
SELECT *
FROM phone_calls
WHERE year = 2023 AND month = 7 AND day = 28 AND duration < 60;
--ADD NAME TO CALL LIST OF CALLERS
SELECT p.name, pc.caller, pc.year, pc.month, pc.day, pc.duration
FROM phone_calls pc
JOIN people p ON pc.caller = p.phone_number
WHERE pc.year = 2023 AND pc.month = 7 AND pc.day = 28 AND pc.duration < 60;
--EXPLORE AIRPORT TO FIND FIFTEVILLE-
...
-- combine all the three witnesses--
SELECT p.name
FROM bakery_security_logs bsl
JOIN people p ON p.license_plate = bsl.license_plate
...
--narrow down from two suspects who is on flight--
SELECT p.name
FROM people p
